...
typedef enum { INFILE, RANDOM, INCREASING, DECREASING,
    INSERTION, MERGE, QUICK1, HEAP, RADIX, SELECTION,
    BUCKET, QUICK2, QUICK3, EXIT}
    MenuOptionTypes;
...
void BucketSort(long *, int);                  /* Bucket Sort */
    ...
    do                   /* Loop executes until done is equal to 1.   */
    {
        ...
        switch(choice)    /* Examine value of user's choice.           */
        {
            ...
            case RADIX:  /* (9) RADIX SORT */
                ...
                break;
            case SELECTION:  /* (10) SELECTION SORT */
                /* Check to make sure there is a list to sort...*/
                if( List != NULL )
                {
                    /* Create a Test List and copy original values into it*/
                    ...
                    CopyList(TestList, List, num);
                    start = clock();               /* get start time */
                    SelectionSort(TestList, num);  /* sort list      */
                    end = clock();                 /* get end time   */
                    ShowResults("1Selection Sort",start,end,num,filename);
                    Check(TestList,num);  /* Check for correct sorting */
                    free(TestList);
                } else {
                    printf ("\nThere is no input list. Specify one!.\n");
            ...
            case BUCKET:  /* (11) BUCKET SORT */
                ...
                if( List != NULL )
                {
                    ...
                    CopyList(TestList, List, num);
                    start = clock();               /* get start time */
                    BucketSort(TestList, num);     /* sort list      */
                    end = clock();                 /* get end time   */
                    ShowResults("1Bucket Sort",start,end,num,filename);
                    Check(TestList,num);  /* Check for correct sorting */
                    free(TestList);
                } else {
                    printf ("\nThere is no input list. Specify one!.\n");
...
PROCEDURE: SelectionSort (*L, n)

This procedure uses the selection-sort method to sort elements
...
void SelectionSort(long *L, int n)
{
    int x;                        /* index of value to be inserted */
    int i, j;                     /* index in for loops */
    ...
    while(j < n)
    {
        ...
        for(i = j; i < n; i++)
        {
            if(L[i] < L[x])    /* find minimum value between index j to n*/
                x = i;
#ifdef TAKE_COUNT
            comparison_count++;
#endif
...
void BucketSort(long *L, int n)
{
    long m;                      /* maximum value in the list */
    Node *list[10]={NULL};       /* array of pointers to node */
    Node *head,*tp;
    int i,j;                     /* index in for loops */
    long d;                  /* will be used to divide each array element */
    long temp;
    ...
    /* Displays user menu choices and returns value of user's choice */
{
    ...
    printf("(11) Quick Sort (pivot as random)  \n");
    printf("(12) Quick Sort (pivot as median)  \n");
    printf("(13) Exit                   \n");
